...
You must activate it every time you open a new Command Prompt window:

venv\Scripts\activate

When it's active, your prompt will show (venv) at the start of the line. That means you're good.
If you get a "cannot be loaded" error, first, 

run: Set-ExecutionPolicy -Scope CurrentUser RemoteSigned in PowerShell, then try again in cmd.

Phase C — Install Libraries
Upgrade pip first
Always upgrade pip before installing packages to avoid dependency errors:

python -m pip install --upgrade pip

Install all required libraries takes 2–5 min

Paste this entire command, it installs everything the app needs in one shot:
...
To confirm app.py is there, type: dir in your cmd window. You should see app.py listed.
